...
//- 0: busca por nombre
//- >0: busca por fd
Descriptor *busca_des(ListaDes *des, char *dato, int tipo)
{
  Descriptor *ptr = des->inicio;
  int descrip;
...
char *sacar_nueva_linea(char *nombre, char tipo)
{
  char *copia;
  int i, n;
...
char* concatenar_archivos(Lista *lista)
{
  ...
  if((nombres = (char *) malloc(MAXSIZE_COLA * sizeof (char))) == NULL)
    printf("error al concatenar los nombres de archivos\n");
  while(ptr != NULL) {
    if(ptr->nombre != NULL) {
      aux = sacar_nueva_linea(ptr->nombre, 'l');
      strcat(nombres," ");
      strcat(nombres,aux);
